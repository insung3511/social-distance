...
def calculate_social_distancing(vid_path, net, output_dir, output_vid, ln1, sleepTime):
    ...
    while True:
        ...
        wholePeople = int(highRisk) + int(low_Risk) + int(safeRisk)
        warnPeople = (int(highRisk) + int(low_Risk))
        ...
        if count != 0:
            ...
            cv2.putText(img, str(now), (10, 26), font, 2, (255, 0, 255), 2)
...
def printRisk(self):
    web_result = ""
    wholePerson = int(highRisk) + int(low_Risk) + int(safeRisk)
