...
class VideoCamera(object):
    def get_frame(self):
        success, FrameImage = self.video.read()
        ret, jpeg = encodingImg
        return jpeg.tobytes() 
...
def calculate_social_distancing(vid_path, net, output_dir, output_vid, ln1, sleepTime):    
    ...
    while True:
        ...
        peopleCnt = (int(highRisk) + int(low_Risk) + int(safeRisk))
        print("People Count: ", peopleCnt, end='')
        
        try:
            noneCompliance = ((int(highRisk) + int(low_Risk)) / peopleCnt) * 100
...
if __name__== "__main__":
    ...
    print("[INFO] Default \"sdc_path\" is ", sdc_path)
    print("[INFO] Default \"sleepTime\" is ", sleepTime)
